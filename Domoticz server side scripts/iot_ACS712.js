...
// V0.6  - February 2019
          // IMPROVEMENT : Don't give up anymore at boot if Domoticz not yet available 
// V0.5  - June 2018
...
var   DZ_AVAILABLE            = false; // Domoticz available status
...
// ******* INIT
console.log("*** " + new Date() + " - Domoticz iot_ACS712 v0.6 starting ***\n");
if( VERBOSE ) console.log("Trying to get Domoticz overall heating index meter...");
var DzConnect = setInterval(function() { 
                    httpDomoticz.get(GetOverallHeatersMeter, function(resp) {   // Get Main Energy Consumption/Forecast Meter
                         resp.on('data', function(JSONoverall){ 
                             ...
                             if( VERBOSE ) console.log("Latest overall heating index meter - HP/HC Consumption/Forecast (Wh): " + USAGE_HP + "/" + USAGE_HC + "/" + FORECAST_HP + "/" + FORECAST_HC);
                             clearInterval( DzConnect );
                             DZ_AVAILABLE = true;
                         });
                    }).on("error", function(e){
                         console.log("Domoticz not available at boot - Can't get overall heating index meter: " + e.message);
                         console.log("Try again in 5 s\n");
                    });
...
setInterval(function(){ // compute overall Heater consumptions and update h/r ratios every n minutes
  ...
  if( Thermostat_setPoint != -150 ) {
      if( !Rollback )  {  // We didn't success the last loop time to reach Domoticz heaters meters, so rollback Main P1 smart meter to last values and do not update this main meter
          ...
          console.log("=== " + new Date() + " - ROLLBACK DONE. One of the Domoticz index meters was unavailable ===");
      ...
      if( Steady && !Rollback ) { // compute and log H ratio      		 
      		 if( DegresDay > 0 /* && ( PreviousIndoorTemp-PreviousOutdoorTemp ) > INDOOR_TEMP_REACHED*/  ) {
